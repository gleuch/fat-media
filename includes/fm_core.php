...
class fatmedia extends imageResize {
  var $reqs = array();
  var $ff_items = array();
  var $ff_dirs = array();
  var $ff_vals = array();
  ...
  */
  function fatmedia() {
    ...
    $this->dir_fs_tmpl = str_replace('//', '/', FF_TEMPLATE_DIR . FF_USE_TEMPLATE .'/');
    $this->dir_tmpl = $this->dir_root . $this->dir_fs_tmpl;
    $this->sortDir = true;
    $this->ff_total = 0;
    $this->dir_date = 0;
    $this->timenow = time();
  ...
  */
  function parseRequest() {
    ...
    if(!empty($_COOKIE['fatmedia_sort'])) $this->reqs['sort'] = $_COOKIE['fatmedia_sort'];
  ...
  */
  function openTemplate($file) {
    ...
    if (is_dir($this->dir_fs_tmpl)) {
      ...
      echo('you need a place to hang your shit. add a fatmedia template.');
    }
  ...
  */
  ...
  function pageTitle() {
    echo FF_NAME.' '.(($this->dir != FF_DATA_DIR) ? FF_SEPARATOR.str_replace( array('data/', '/', '-', '_'), array('', '/', ' ', ' '), cleanDirname($this->dir)) : (defined('FF_ANTI_CLOUD_MSG') ? FF_SEPARATOR.FF_ANTI_CLOUD_MSG : ''));
  }
